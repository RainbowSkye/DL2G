...
def get_arch(opt):
    from Uformer.model import Uformer, UNet
    ...
    if arch == 'UNet':
        ...
        model_restoration = Uformer(img_size=opt.get("train_ps"),embed_dim=32,win_size=8,token_projection='linear',token_mlp='leff',
            depths=[1, 2, 8, 8, 2, 8, 8, 2, 1],modulator=True,dd_in=opt.get("dd_in"))
    else:
