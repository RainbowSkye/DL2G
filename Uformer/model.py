...
class WindowAttention(nn.Module):
    def __init__(self, dim, win_size,num_heads, token_projection='linear', qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        super().__init__()
        self.dim = dim
        self.win_size = win_size  # Wh, Ww default set to 8*8
        ...
        # print(torch.zeros((2 * win_size[0] - 1) * (2 * win_size[1] - 1)).shape)     # 225*225
        # YZL: when test, it is next
        # self.relative_position_bias_table = nn.Parameter(
        #     torch.zeros((2 * win_size[0] - 1) * (2 * win_size[1] - 1), num_heads))  # 2*Wh-1 * 2*Ww-1, nH
...
class LeWinTransformerBlock(nn.Module):
    ...
    def forward(self, x, mask=None):
        B, L, C = x.shape
        H = int(math.sqrt(L))   # if L not int sqrted, it will be wrong
        # if H//self.win_size !=0:
        #     H = H-(H%self.win_size)
        W = int(math.sqrt(L))
        # if W//self.win_size !=0:
        #     W = W-(W%self.win_size)
        ...
        shortcut = x
        x = self.norm1(x)
        # x = x.view(B, H+(H%self.win_size), W+(W%self.win_size), C)
        # x = x.view(B, (H+(H%self.win_size))+(math.sqrt(L)-H_ori), (W+(W%self.win_size))+(math.sqrt(L)-W_ori), C)  # must be int
        x = x.view(B, H, W, C)
